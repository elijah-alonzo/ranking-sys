...
class AdviserEvaluationForm
{
    public static function getSchema(): array
    {
        $questions = EvaluationFormModel::getQuestionsForEvaluator('adviser');
        $sections = [];
        ...
        foreach ($groupedQuestions as $domain => $domainQuestions) {
            ...
            $sections[] = Section::make($domain)
                ->description('Please rate each statement based on the student\'s performance')
                ->schema($fields)
                ->collapsible()
                ->columns(1);
        }
