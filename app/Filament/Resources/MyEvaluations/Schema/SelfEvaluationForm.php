...
class SelfEvaluationForm
{
    public static function getSchema(): array
    {
        $questions = EvaluationFormModel::getQuestionsForEvaluator('self');
        $sections = [];
        ...
        foreach ($groupedQuestions as $domain => $domainQuestions) {
            ...
            $sections[] = Section::make($domain)
                ->description('Please rate each statement based on your own performance')
                ->schema($fields)
                ->collapsible()
                ->columns(1);
        }
